...
function App() {
  ...
  return <>
    ...
    <header className="site-header"><a className="brand" href="#top"><span className="brand-mark">HB</span><span>HARSH / BHARDWAJ</span></a><nav>{['work', 'technical-skills', 'about'].map((id) => <button className={active === id ? 'active' : ''} onClick={() => scrollTo(id)} key={id}>{id === 'technical-skills' ? 'Skills' : id}</button>)}</nav><button className="header-cta" onClick={() => scrollTo('contact')}>Let’s talk <span>↗</span></button></header>
    <main id="top">
      <section className="hero section-shell"><div className={reveal('hero-copy', 'hero-copy reveal-left')} data-reveal="hero-copy"><p className="eyebrow"><span className="status-dot" /> Backend engineer / Noida, IN</p><h1>Systems that stay <em>sharp</em><br />under pressure.</h1><p className="hero-intro">I’m Harsh, a backend engineer at TCS turning complex business logic into resilient, observable products.</p><div className="hero-actions"><button className="button button-primary" onClick={() => scrollTo('work')}>Explore my work <span>↓</span></button><a className="text-link" href="Harsh_Bhardwaj_cv.pdf" download>Download CV <span>↗</span></a></div></div><div className={reveal('hero-art', 'hero-art reveal-scale')} data-reveal="hero-art"><div className="orbit orbit-one" /><div className="orbit orbit-two" /><div className="orbit orbit-three" /><div className="core-node"><strong>HB</strong><small>BUILD / DEBUG / REPEAT</small></div><span className="float-label label-java">JAVA <b>01</b></span><span className="float-label label-redis">REDIS <b>02</b></span><span className="float-label label-rag">RAG <b>03</b></span><div className="scan-line" /></div><div className={reveal('hero-foot', 'hero-foot reveal-up')} data-reveal="hero-foot"><span>01 — Selected portfolio</span><span>Scroll to inspect ↓</span></div></section>
