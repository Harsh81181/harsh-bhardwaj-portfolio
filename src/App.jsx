...
function App() {
  ...
  return <>
    ...
    <header className="site-header"><div className="header-identity"><a className="brand" href="#top"><span className="brand-mark">HB</span><span>HARSH / BHARDWAJ</span></a><div className="header-socials" aria-label="Social profiles">{socials.map(([label, href]) => <a href={href} target="_blank" rel="noreferrer" aria-label={label} title={label} key={label}><span>{label === 'GitHub' ? 'GH' : label === 'LinkedIn' ? 'in' : 'LC'}</span></a>)}</div></div><nav>{['work', 'technical-skills', 'about'].map((id) => <button className={active === id ? 'active' : ''} onClick={() => scrollTo(id)} key={id}>{id === 'technical-skills' ? 'Skills' : id}</button>)}</nav><button className="header-cta" onClick={() => scrollTo('contact')}>Let’s talk <span>↗</span></button></header>
    <main id="top">
